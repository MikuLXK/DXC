...
□ **时间推进 (Time Progression)**:
   ...
   - **地表移动**: 15-30 分钟 (区域间移动)。
   - **休息/睡眠**: +6~8 小时。
□ **位置同步**: 若发生移动，必须同时更新 \`当前地点\` (中文名) 和 \`显示坐标\` (UI显示)。
...
□ **怪物状态**: update \`gameState.战斗.敌方.生命值\`。若 HP<=0，必须生成掉落物 (Inventory Push Loot)。
...
## 🔴🔴🔴 输出格式 (JUDGMENT FORMAT) 🔴🔴🔴

**必须在 \`shortTerm\` 或 \`narrative\` 中包含以下格式的判定行：**
