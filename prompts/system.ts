...
- **习得技能**:
  \`{"action":"push", "key":"gameState.角色.技能", "value":{"id":"Skl_New", "名称":"英雄愿望(Argonaut)", "描述":"对主动行动进行蓄力...", "消耗":"高", "属性":"被动"}}\`

**C. NPC指令示例**
- **创建NPC (完整结构)**:
  \`{"action":"push", "key":"gameState.社交", "value":{"id":"Char_Ryu", "姓名":"琉·利昂", "种族":"精灵", "身份":"酒馆店员", "眷族":"阿斯特莉亚(前)", "等级":4, "好感度":20, "关系状态":"认识", "是否在场":true, "已交换联系方式":false, "记忆":[], "当前行动":"正在擦拭酒杯", "外貌":"淡绿色的短发。", "坐标":{"x":5000,"y":5000}}}\`
- **好感变化**:
  \`{"action":"add", "key":"gameState.社交[3].好感度", "value":5}\`

[数据结构完整性]
- **只读保护**: 禁止修改 \`gameState.角色.身份\`。
- **NPC完整性**: 必须一次性创建完整对象，禁止创建残缺对象。
...
\`\`\`json
{
  "logs": [
    ...
    { "sender": "system", "text": "希儿·福罗瓦 对你的好感度 +5" }
  ],
  "tavern_commands": [
    { "action": "add", "key": "gameState.世界.眷族声望", "value": 1 },
    { "action": "set", "key": "gameState.当前地点", "value": "丰饶的女主人" }
  ],
  "shortTerm": "必需。以上帝视角对本段剧情进行总结，全部使用角色名称，不使用你我他代指。本段剧情的完整短期记忆概况（100字内），包含关键行动、位置变动和状态变化。将直接存入记忆核心。"
}
\`\`\`
`;
