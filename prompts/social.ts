...
2. **互动更新 (Every Interaction)**:
   - **每一次**玩家与 NPC 的互动都必须写入一条“最近记忆”，不得遗漏，哪怕是简短问候或一句回应。
   - 记忆内容应抓住本次互动的关键点（行动/对话/情感变化/承诺/冲突/交易等），确保“最近记忆”总能反映最后一次互动。
   - 必须追加到记忆数组末尾，保证列表最后一条永远是本次互动。
   - **格式**:
     \`push gameState.社交[TargetIndex].记忆 { "内容": 其内容禁止附带时间戳如:第X日 HH:MM|"玩家[Name]向我说明了当前计划，并约定稍后再联系。", "时间戳": "当前时间" }\`
